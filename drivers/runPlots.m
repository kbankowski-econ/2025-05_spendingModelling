...
for aModel = modelList
    ...
    resultsProc.(aModel).endoStruct.model = databank.fromArray( ...
        resultsRaw.oo_.endo_simul', ...
        resultsRaw.M_.endo_names, ...
        dataRange(1) ...
    );
    
    resultsProc.(aModel).ssStruct.model = databank.fromArray( ...
    ...
    for aParam = string(reshape(resultsRaw.M_.param_names, 1, []))
        resultsProc.(aModel).paramStruct.model.(aParam) = resultsRaw.M_.params(strcmp(aParam, resultsRaw.M_.param_names));
    end

    % Calculate IRF transformations
    serIndex = cellfun(@(x) any(endsWith(x, {'effshock', 'effgeshock'})), resultsRaw.M_.endo_names);

    resultsProc.(aModel).irfStruct.model = databank.copy( ...
        resultsProc.(aModel).endoStruct.model, ...
    ...
    resultsProc.(aModel).irfStruct.model = databank.copy( ...
        ...
        "TargetDb", resultsProc.(aModel).irfStruct.model ...
    );
...
function vertModelComparison(resultsProc, VarListToPlot, modelList, outputFileName)
    ...
    for aVar = VarListToPlot
        ...
        for aModel = modelList % for each panel
            % Plotting the data
            plotData = resultsProc.(aModel).irfStruct.model.(aVar){dataRange}; % Accessing the specific variable for the current model
