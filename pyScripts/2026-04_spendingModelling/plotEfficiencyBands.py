"""
WP calibration figure: spending-efficiency gaps over time — interquartile bands
by income group (advanced economies, emerging markets, low-income developing
countries) for infrastructure, health and education (3x1 panel).

Adapted from the Jamaica deck's plotEfficiencyBandsJAM.py, with Jamaica removed.
Its purpose here is to justify the efficiency gaps calibrated in the working
paper: the shaded band is the 25th-75th percentile of eff_gap across each
group's economies in each year, the centre line is the group median, and the
labelled circle marks each group's 2023 value (the calibration reference). The
calibrated AE/EMDE gaps in the paper can be read against these empirical ranges.

Data: IMF staff efficiency estimates (2025-04-14),
[SECTOR]_inefficiency-scores.csv, eff_gap column (Developer copy).
"""
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots
from fiscal_common import load_chart_config, get_project_root
...
def main():
    ...
    for col, sector in enumerate(SECTORS, start=1):
        ...
        for (idx, (v, colr, _)), ly in zip(present, label_ys):
            fig.add_trace(go.Scatter(
    ...
    figures_dir = get_project_root() / "docs" / "2026-06_wp-imf" / "figures"
