"""
Jamaica case deck (local copy): 1x2 panel of the additional EMDE output gain in
2050 from gradually closing spending-efficiency gaps, for infrastructure (left)
and human capital (right), at two closure horizons. Bars show the baseline
(low-initial-gap) response; circle markers show the counterpart when the initial
gap is higher ("low" model variant). Ported from pyScripts/2026-04_spendingModelling
so the deck's bar charts can be tailored for Jamaica independently.
"""
import pandas as pd
