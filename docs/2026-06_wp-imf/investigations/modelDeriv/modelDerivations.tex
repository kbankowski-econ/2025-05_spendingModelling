...
retail layer (the final good's variety demand and the retailers' Calvo price
setting), the intermediate firm's cost minimization, and
...
constant $g$ in their model counterparts below comes from stationarization (see
...
\begin{itemize}
    ...
    $1/g$ appears.} The two $\lambda$'s sit at different dates, so their
    trends do not cancel: $\lambda_{t+1}/\lambda_t =
    (\hat\lambda_{t+1}/\hat\lambda_t)/g$.
...
%========================================================================
\section{Retailers}\label{sec:pricing}
%========================================================================

This section covers the retail layer: first the final good's demand for retail
varieties and the aggregate price index, then the retailers' Calvo price
setting --- the two auxiliary recursions for $x_{1,t}$ and $x_{2,t}$, the
reset-price condition $\epsilon x_{1,t} = (\epsilon-1)x_{2,t}$, and the
price-index law of motion.

\subsection{The Final Good and the Demand for Varieties}

The final output good is a CES aggregate of a continuum of retail varieties:
\begin{equation}\label{eq:cesagg}
    Y_t^{d} = \left[\int_0^1 y_t(i)^{\frac{\epsilon-1}{\epsilon}}\,di
    \right]^{\frac{\epsilon}{\epsilon-1}}, \qquad \epsilon > 1.
\end{equation}
The final-good producer is competitive; its problem is:
\begin{equation*}
    \max_{y_t(i)} \; P_t \left[\int_0^1
    y_t(i)^{\frac{\epsilon-1}{\epsilon}}\,di\right]^{\frac{\epsilon}{\epsilon-1}}
    - \int_0^1 P_t(i)\, y_t(i)\, di.
\end{equation*}
The FOC for a typical variety $i$ is:
\begin{equation*}
    P_t\,\frac{\epsilon}{\epsilon-1}\left[\int_0^1
    y_t(i)^{\frac{\epsilon-1}{\epsilon}}\,di\right]^{\frac{\epsilon}{\epsilon-1}-1}
    \frac{\epsilon-1}{\epsilon}\, y_t(i)^{-\frac{1}{\epsilon}} = P_t(i).
\end{equation*}
The two elasticity factors cancel, and making note of the definition of the
aggregate \eqref{eq:cesagg}, the bracket term is just
$\left(Y_t^{d}\right)^{1/\epsilon}$. Solving for $y_t(i)$:
\begin{equation}\label{eq:demand}
    y_t(i) = \left(\frac{P_t(i)}{P_t}\right)^{-\epsilon} Y_t^{d}.
\end{equation}
\eqref{eq:demand} says the demand for each variety is proportional to total
demand and decreasing in the variety's relative price; $\epsilon$ is the price
elasticity of demand, which is what gives the retailers their pricing power.

For the price index, note that the aggregator is homogeneous of degree one, so
the competitive final-good producer earns zero profits: nominal revenue equals
nominal spending, $P_t Y_t^{d} = \int_0^1 P_t(i)\,y_t(i)\,di$. Plugging in the
demand \eqref{eq:demand}:
\begin{equation*}
    P_t Y_t^{d} = \int_0^1 P_t(i)^{1-\epsilon} P_t^{\epsilon}\, Y_t^{d}\, di.
\end{equation*}
Pulling out of the integral the terms that do not depend on $i$ and
simplifying:
\begin{equation}\label{eq:priceindex}
    P_t = \left[\int_0^1 P_t(i)^{1-\epsilon}\,di\right]^{\frac{1}{1-\epsilon}}.
\end{equation}
Neither \eqref{eq:demand} nor \eqref{eq:priceindex} is a model equation: the
variety level is aggregated out of the Dynare system, and both appear in the
paper's Section~2.2 exactly as derived here.

\intuition{Why do we have both retailers and a final-good producer --- aren't they the same?}{%
No: their roles are mutually exclusive. The final-good producer is a
competitive, zero-profit bundler with no market power and no interesting
decision; its entire contribution is the demand curve \eqref{eq:demand} and
the price index \eqref{eq:priceindex}, and it keeps the rest of the model
denominated in a single good. Retailers are monopolistic price setters, one
variety each, and the downward-sloping demand they face is exactly what gives
the Calvo friction something to bite on. The two cannot sit in one firm: a
price setter needs a demand curve for its product generated by someone
\emph{else's} substitution behavior --- if the retailer were also the
aggregator, it would be choosing the demand for its own good. What could be
eliminated is the final-good producer, by merging it into the
\emph{household}: put the CES basket directly into preferences and expenditure
minimization across varieties delivers the identical demand curve and price
index (the reference notes' aside ``Putting the CES Aggregate in
Preferences''). The final-good firm is simply the tidier bookkeeping device.
The model's three production layers then each carry one function:
intermediate producers generate the technology rents ($\mu^{p}$), retailers
generate the nominal rigidity ($\epsilon$, Calvo), and the final-good
producer is the zero-profit veil that generates the demand system.}
...
varieties, and sell variety $i$ at the nominal price $P_t(i)$, facing the demand
curve \eqref{eq:demand}.
...
\intuition{Who sets prices --- retailers or intermediate producers?}{%
Both layers have a price, but only the retailers \emph{set} prices in the New
Keynesian sense. Intermediate producers make no dynamic pricing decision: they
charge the fixed gross markup $\mu^{p}$ over marginal cost period by period,
flexibly --- a static rule with no state and no friction, embedded in the
factor demands rather than written as its own equation
(Section~\ref{sec:costmin}). They are flexible-price monopolistic competitors
whose pricing role is to generate the rent that rewards technology adoption.
Retailers choose \emph{nominal} prices subject to the Calvo friction, and
everything nominal lives in their layer: the reset-price system, inflation
dynamics, the price-index law of motion, price dispersion, and hence the
transmission of monetary policy. The final-good producer is competitive and
sets no price at all; the price level emerges from aggregating retail prices.
The division of labor mirrors the two markups (see the aside in
Section~\ref{sec:costmin}): flexible fixed-markup pricing upstream pins the
real profit share feeding the innovation block, sticky pricing downstream
generates the nominal rigidity --- and keeping them in separate layers,
following Anzoategui et al.\ (2019), lets the model calibrate the two
independently.}
...
The price index \eqref{eq:priceindex} aggregates $P_t^{1-\epsilon} = \int_0^1 P_t(i)^{1-\epsilon}di$.
...
produces output using private capital and effective labor. It takes the factor
prices $r_t^{k}$ and $w_t$, the public stock $K_{t-1}^{GI}$, and adopted
technology $A_{t-1}$ as given. Public infrastructure is an external productive
input to the firm. Its technology is
\begin{equation}\label{eq:tech}
    Y_t(i) = A_{t-1}^{\vartheta-1}K_{t-1}(i)^{\alpha}
    \left(K_{t-1}^{GI}\right)^{\alpha_G}N_t(i)^{1-\alpha}.
\end{equation}
...
\begin{equation}\label{eq:cmconstraint}
    A_{t-1}^{\vartheta-1}K_{t-1}(i)^{\alpha}
    \left(K_{t-1}^{GI}\right)^{\alpha_G}N_t(i)^{1-\alpha}
    \;\ge\; \bar{Y}_t(i).
\end{equation}
...
\begin{equation*}
    ...
    + \xi_t\left[A_{t-1}^{\vartheta-1}K_{t-1}(i)^{\alpha}
      \left(K_{t-1}^{GI}\right)^{\alpha_G}N_t(i)^{1-\alpha}
      - \bar{Y}_t(i)\right].
\end{equation*}
The FOC are:
...
much the minimized cost rises if the firm must produce one more unit. That is the
firm's \emph{real marginal factor cost}. Conditional on the external public
capital stock, the privately chosen inputs exhibit constant returns to scale.
...
$(mc_t-mc_t/\mu^{p})Y_t$ --- transaction revenue minus factor cost. It is the
fixed-markup profit of the intermediate producer. The three equations together
--- labor demand, Calvo recursion, and technology value --- pin down this
interpretation.
...
\begin{aligned}
    ...
    + (1-\rho_A)\alpha_{HA}\,\ln\frac{H_{t-1}}{H^{SS}} \\
    &{}+ (1-\rho_A)\alpha_{RD}\,\ln\frac{\left(1-e^{GRD}_{t-1}\right) G^{RD}_{t-1}}
    {\left(1-e^{GRD}\right) G^{RD,SS}},
\end{aligned}
\end{equation}
in the model:
\modelcode{ln(Z/STEADY\_STATE(Z)) = rho\_A*ln(Z(-1)/STEADY\_STATE(Z))\\
\hspace*{2em}+(1-rho\_A)*alphaRD*ln((1-eGRD(-1))*Grd(-1)/((1-eGRD\_ss)*STEADY\_STATE(Grd)))\\
\hspace*{2em}+(1-rho\_A)*alphaHA*ln(H(-1)/STEADY\_STATE(H));}
Three remarks. First,
$\alpha_{HA}$ and $\alpha_{RD}$ are the \emph{long-run elasticities} of created
technology; the corresponding per-period loadings are
$(1-\rho_A)\alpha_{HA}$ and $(1-\rho_A)\alpha_{RD}$.
...
Between $t-1$ and $t$, a fraction $\phi$ of all technologies survives --- the
survival process applies to adopted and unadopted technologies alike --- and
each surviving unadopted technology becomes adopted with probability $q_t$. In
levels:
...
$1+\gamma^{a} = g^{(1-\alpha-\alpha_G)/(\vartheta-1)}$, faster than output. Substituting
...
\intuition{Where does $1+\gamma^{a} = g^{(1-\alpha-\alpha_G)/(\vartheta-1)}$ come from?}{%
It is the balanced-growth requirement on the production function. On the
balanced-growth path, output and both capital stocks grow at the gross rate $g$
and labor is stationary, so taking growth factors of the production function gives
\[
    g = \left(1+\gamma^{a}\right)^{\vartheta-1} g^{\alpha+\alpha_G}
    \qquad\Longrightarrow\qquad
    1+\gamma^{a} = g^{(1-\alpha-\alpha_G)/(\vartheta-1)},
\]
the code's \texttt{gammaa = g\^{}((1-alpha-alphaG)/(vartheta-1))-1}
(\texttt{models/modelTemplate.mod}). Private and public capital deliver
$g^{\alpha+\alpha_G}$ of the required growth, so the love-of-variety TFP term
must supply the remaining $g^{1-\alpha-\alpha_G}$. Because its exponent
$\vartheta-1$ is small, $A$ still grows faster than output. The exact exponent
differs across the AE and EMDE calibrations through $\alpha_G$.}

\subsection{The Value of an Adopted Technology}

Start from one adopted variety and ask what it is worth to its owner. It
entitles the owner to the per-variety profit flow
\begin{equation}\label{eq:pidef}
    \pi_t = \frac{1}{A^{\ell}_{t-1}}\,\frac{\mu^{p}-1}{\mu^{p}}\,mc_t\,Y^{\ell}_t,
\end{equation}
the total intermediate-producer profit flow \eqref{eq:profit} divided by the
$A^{\ell}_{t-1}$ varieties active at $t$. The value of the technology is the
expected present value of this rent stream over its remaining lifetime:
\begin{equation}\label{eq:vsum}
    v_t = \mathbb{E}_t \sum_{s=0}^{\infty} \phi^{s}\,
    \mathit{SDF}^{\ell}_{t,t+s}\,\pi_{t+s},
\end{equation}
where $\mathit{SDF}^{\ell}_{t,t+s} = \mathit{SDF}^{\ell}_{t+1}\cdots
\mathit{SDF}^{\ell}_{t+s}$ (with $\mathit{SDF}^{\ell}_{t,t}=1$) is the
household's compounded discount factor (Subsection~\ref{subsec:sdf}). Each term
in \eqref{eq:vsum} is discounted twice: by the compounded SDF and by
$\phi^{s}$, the probability that the technology is still alive $s$ periods out
--- obsolescence acts like an addition to the discount rate. In steady state
the sum is geometric, $v = \pi/(1-\phi\,\mathit{SDF})$, the closed form in
\texttt{models/Steady\_states\_solution.m} (with the $1+\gamma^{a}$ factors
from detrending).

Dynare handles recursions, not infinite sums, so --- exactly as with the
pricing sums of Section~\ref{sec:pricing} --- split off the $s=0$ term of
\eqref{eq:vsum} and use $\mathit{SDF}^{\ell}_{t,t+s} =
\mathit{SDF}^{\ell}_{t+1}\,\mathit{SDF}^{\ell}_{t+1,t+s}$:
...
$\pi_t$, itself microfounded in Section~\ref{sec:costmin} as the fixed markup
rent earned by the intermediate producer.
...
\intuition{Going back from \eqref{eq:vsmall} to \eqref{eq:vsum}: doesn't compounded discounting make the limit term vanish automatically?}{%
Iterating \eqref{eq:vsmall} forward reproduces \eqref{eq:vsum} plus the limit
term $\lim_{s\to\infty}\phi^{s}\,\mathbb{E}_t[\mathit{SDF}^{\ell}_{t,t+s}\,
v_{t+s}]$: the sum always satisfies the recursion, but the recursion implies
the sum only when this term vanishes (the no-bubble condition). For the
solutions the model actually produces it does vanish: on a path converging to
the balanced-growth path, each one-step level discount is
$\mathit{SDF}^{\ell} \approx \beta/g$ while $v^{\ell}_{t+s}$ grows like
$g^{s}$, so the term is roughly $\phi^{s}(\beta/g)^{s}g^{s}v_t =
(\beta\phi)^{s}v_t \to 0$, dying geometrically at rate $\beta\phi$ per
quarter. The mechanism is that the value grows \emph{more slowly} than the
discount shrinks --- the $g$'s cancel and $\beta\phi<1$ survives. But
compounding alone does not kill the term for every solution of the recursion:
a bubble $b_t = \phi\,\mathbb{E}_t[\mathit{SDF}_{t+1}b_{t+1}]$ grows at
exactly the reciprocal of the discount, so
$\phi^{s}\,\mathbb{E}_t[\mathit{SDF}_{t,t+s}\,b_{t+s}] = b_t$ for every $s$
--- constant, not vanishing. Discounting beats anything that grows slower than
$1/(\phi\,\mathit{SDF})$ per period; a bubble is built to grow at exactly that
rate. What rules bubbles out is economics (household transversality) and, in
practice, the solution method: the model is solved by perfect foresight on a
finite horizon with terminal values pinned to the steady state, which
mechanically selects the bubble-free path.}
...
$\mathit{SDF}^{\ell}_{t+1} = \mathit{SDF}_{t+1}/g$; and the technology-stock
ratio converts as $A^{\ell}_{t-1}/A^{\ell}_t =
(A_{t-1}/A_t)/(1+\gamma^{a})$. Substituting into \eqref{eq:Vlevel} and dividing by
$\Lambda_t$, the $g$ from the SDF cancels against the trend growth
...
An unadopted technology is an option with a random exercise date. While it
remains unadopted, its adopter pays the outlay $s_t$ each period; adoption
arrives with hazard $q_t$, at which point the technology becomes an adopted
one and earns the rents of \eqref{eq:vsum}. Because the exercise date is
random, the weights must track the probability of still waiting: let
$Q_{t,s} = \prod_{k=0}^{s-1}(1-q_{t+k})$, with $Q_{t,0}=1$, be the probability
that a technology unadopted at $t$ is still unadopted at $t+s$ (conditional on
surviving). The value of the option is the expected present value of the
waiting costs and the prize:
\begin{equation}\label{eq:jsum}
    j_t = -\,\mathbb{E}_t \sum_{s=0}^{\infty} \phi^{s}\,
    \mathit{SDF}^{\ell}_{t,t+s}\,Q_{t,s}\,s_{t+s}
    \;+\; \mathbb{E}_t \sum_{s=0}^{\infty} \phi^{s+1}\,
    \mathit{SDF}^{\ell}_{t,t+s+1}\,Q_{t,s}\,q_{t+s}\,v_{t+s+1}.
\end{equation}
The first sum is the discounted stream of adoption outlays paid while waiting
(unadopted with probability $Q_{t,s}$, alive with probability $\phi^{s}$). The
second is the discounted prize: conversion at the end of period $t+s$ has
probability $q_{t+s}Q_{t,s}$, the technology survives into $t+s+1$ (hence
$\phi^{s+1}$), and is from then on worth $v_{t+s+1}$ --- substituting
\eqref{eq:vsum} turns the prize into the rent stream earned only from the
adoption date onward. Each term compounds three risks: death ($\phi$ powers),
still waiting ($Q$), and time preference (the compounded SDF). In steady state
the sums collapse to
$j = (\phi\,\mathit{SDF}\,q\,v - s)/(1-\phi\,\mathit{SDF}(1-q))$.

Splitting off the $s=0$ terms and using $Q_{t,s} = (1-q_t)\,Q_{t+1,s-1}$ rolls
\eqref{eq:jsum} into the one-period bet the model carries:
...
\end{equation}
Pay $s_t$ today; conditional on surviving, the adopter holds the prize
$v_{t+1}$ with probability $q_t$ and still holds the option $j_{t+1}$
otherwise.
...
\begin{itemize}
    ...
    level SDF equals the detrended one divided by $g$; the two growth
...
by = b/y;}
Tax rates equal their steady-state values plus independent tax-rate shocks, while
transfers respond to the lagged deviation of debt-to-GDP from target:
\modelcode{tauc = taucss+epsi\_tauc;\\
tauw = tauwss+epsi\_tauw;\\
T-STEADY\_STATE(T) = rho\_T*(T(-1)-STEADY\_STATE(T))\\
\hspace*{2em}+(1-rho\_T)*(-gamma\_d\_T*eTaux*(by(-1)-byss)*ydss);}
The minus sign in the transfer rule means transfers fall when debt is above
target. Tax rates do not respond to debt, so transfers are the only active fiscal
feedback. In non-budget-neutral experiments, the auxiliary dummy
\texttt{eTaux} equals zero throughout the spending path and switches off the
transfer response to debt; it returns to one afterward so the transfer feedback
restores debt stability.

\subsection{Monetary Policy and the Government Financing Rate}

The central bank directly sets the nominal rate used for both monetary policy
and government financing through a Taylor rule with smoothing. Following
Gertler and Karadi (2011), minus the log deviation of the retail price markup
proxies for the output gap. Because that markup is the inverse of real marginal
cost, the rule is:
\modelcode{R/Rss = (R(-1)/Rss)\^{}rho\_R\\
\hspace*{2em}*((PI/Piss)\^{}gamma\_pi*(mc/STEADY\_STATE(mc))\^{}gamma\_y)\^{}(1-rho\_R)*exp(epsi\_MP);}
...
The output trend grows at the constant calibrated gross rate $g$:
\[
    \Lambda_t=g\Lambda_{t-1}.
\]
The model therefore carries $g$ as a parameter wherever stationarization links
trending quantities dated in different periods; it contains no separate
trend-growth process or shock.
