...
\intuition{Can \eqref{eq:jsmall} also be written as an infinite sum?}{%
Yes, with one extra ingredient: because adoption is a \emph{random} date, the
weights must track the probability of still waiting. Let
...
surviving). Iterating \eqref{eq:jsmall} forward:
\[
...
\]
An unadopted technology is an option with a random exercise date, adoption
arriving with hazard $q$. The first sum is the expected discounted stream of
adoption outlays paid while waiting (unadopted with probability $Q_{t,s}$,
alive with probability $\phi^{s}$). The second is the expected discounted
prize: conversion at the end of period $t+s$ has probability
$q_{t+s}Q_{t,s}$, the technology survives into $t+s+1$ (hence $\phi^{s+1}$),
and is from then on worth $v_{t+s+1}$ --- substituting \eqref{eq:vsum} turns
the prize into the rent stream earned only from the adoption date onward.
Each term thus compounds three risks: death ($\phi$ powers), still waiting
($Q$), and time preference (the compounded SDF). In steady state the sums
collapse to $j = (\phi\,\mathit{SDF}\,q\,v - s)/(1-\phi\,\mathit{SDF}(1-q))$,
the recursion solved directly.}
