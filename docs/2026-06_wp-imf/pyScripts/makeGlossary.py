...
ENDOGENOUS = [
    ("Households", [
        ...
        (r"$\hat{I}_t$",      "Private investment",                       "Ip"),
        (r"$\hat{K}_t$",      "Private capital",                          "Kp"),
        (r"$\hat{\lambda}_t$", "Marginal utility of consumption",        "lambda"),
    ...
    ("Government: fiscal and monetary policy", [
        (r"$\hat{G}_t^{C}$",  "Government consumption",                   "Gc"),
        (r"$\hat{I}_t^{GI}$", "Public infrastructure investment",         "Igi"),
        (r"$\hat{K}_t^{GI}$", "Public infrastructure capital",            "Kg"),
        (r"$\hat{I}_t^{GE}$", "Public education and health investment",   "Ige"),
        (r"$\hat{K}_t^{GE}$", "Public education and health capital",      "Kge"),
        (r"$\hat{G}_t^{RD}$", r"Public R\&D spending",                    "Grd"),
        (r"$\hat{G}_t$",      "Total government spending",                "G"),
        (r"$\hat{B}_t$",      "Public debt",                              "b"),
        (r"$b_t$",            "Debt-to-GDP ratio",                        "by"),
        (r"$\hat{T}_t$",      "Lump-sum transfers",                       "T"),
    ...
    ("Market clearing and equilibrium", [
        (r"$\hat{Y}_t^{d}$",  "Aggregate demand",                         "yd"),
...
PARAMETERS = [
    ...
    ("Government: fiscal and monetary policy", [
        ...
        (r"$\gamma_y$",         "Policy response to the output-gap proxy",      "gamma_y"),
        ...
        (r"$\hat G^{C,SS}/\hat Y^{d,SS}$",  "Government consumption, share of GDP",       "Gcy"),
        (r"$\hat I^{GI,SS}/\hat Y^{d,SS}$", "Infrastructure investment, share of GDP",    "Igiy"),
        (r"$\hat I^{GE,SS}/\hat Y^{d,SS}$", "Human-capital investment, share of GDP",     "Igey"),
        (r"$\hat G^{RD,SS}/\hat Y^{d,SS}$", "R\\&D spending, share of GDP",               "Grdy"),
...
# (stationarized paper form; g is the gross trend growth rate). The pinned scale
...
EQNS = {
    # Households
    "C":        r"\tfrac{1}{\hat C_t}=\hat\lambda_t(1+\tau^c_t)",
    "L":        r"\omega(L_t+E_t)^{\varphi}=\hat\lambda_t(1-\tau^w_t)\hat w_t H_{t-1}",
    "N":        r"N_t=H_{t-1}L_t",
    "H":        r"H_t=(1-\delta^h)H_{t-1}+\chi E_t^{\gamma}(\hat K^{GE}_{t-1})^{\mu}",
    "E":        r"\omega(L_t+E_t)^{\varphi}=\lambda^H_t\,\chi\gamma\,E_t^{\gamma-1}(\hat K^{GE}_{t-1})^{\mu}",
    "Ip":       r"1=\beta\,\mathbb{E}_t\!\big[\tfrac{\hat\lambda_{t+1}}{\hat\lambda_t}(1-\delta+r^k_{t+1})/g\big]",
    "Kp":       r"g\hat K_t=(1-\delta)\hat K_{t-1}+\hat I_t",
    "rk":       r"r^k_t=\alpha\,(mc_t/\mu^{p})\,\hat Y_t/\hat K_{t-1}",
    "w":        r"\tfrac{\hat K_{t-1}}{N_t}=\tfrac{\alpha}{1-\alpha}\tfrac{\hat w_t}{r^k_t}",
    "lambda":   r"\hat\lambda_t=\beta\,\mathbb{E}_t\!\big[\hat\lambda_{t+1}R_t/(g\Pi_{t+1})\big]",
    "lambda_H": r"\lambda^H_t=\beta\,\mathbb{E}_t\!\big[\hat\lambda_{t+1}(1-\tau^w_{t+1})\hat w_{t+1}L_{t+1}+\lambda^H_{t+1}(1-\delta^h)\big]",
    "SDF":      r"\mathit{SDF}_t=\beta\hat\lambda_t(1+\tau^c_t)/[\hat\lambda_{t-1}(1+\tau^c_{t-1})]",
    # Production and technology
    "y":        r"\hat Y_t=\hat A_{t-1}^{\vartheta-1}\hat K_{t-1}^{\alpha}(\hat K^{GI}_{t-1})^{\alpha_G}N_t^{1-\alpha}",
    "mc":       r"\hat w_t=(1-\alpha)\,(mc_t/\mu^{p})\,\hat Y_t/N_t",
    "PI":       r"1=\theta_p(\Pi_{t-1}^{\chi_p}/\Pi_t)^{1-\epsilon}+(1-\theta_p)(\Pi^*_t)^{1-\epsilon}",
    "PIstar":   r"\epsilon\,x_{1,t}=(\epsilon-1)\,x_{2,t}",
    "x1":       r"x_{1,t}=\hat\lambda_t mc_t \hat Y^d_t+\beta\theta_p\,\mathbb{E}_t(\Pi_t^{\chi_p}/\Pi_{t+1})^{-\epsilon}x_{1,t+1}",
    "x2":       r"x_{2,t}=\hat\lambda_t\Pi^*_t \hat Y^d_t+\beta\theta_p\,\mathbb{E}_t(\Pi_t^{\chi_p}/\Pi_{t+1})^{1-\epsilon}\tfrac{\Pi^*_t}{\Pi^*_{t+1}}x_{2,t+1}",
    "A":        r"(1+\gamma^{a})\hat A_t=q_t\phi(\hat Z_{t-1}-\hat A_{t-1})+\phi\hat A_{t-1}",
    "Z":        r"\log\tfrac{\hat Z_t}{\hat Z^{SS}}=\rho_A\log\tfrac{\hat Z_{t-1}}{\hat Z^{SS}}+(1-\rho_A)\alpha_{HA}\log\tfrac{H_{t-1}}{H^{SS}}+(1-\rho_A)\alpha_{RD}\log\tfrac{(1-e^{GRD}_{t-1})\hat G^{RD}_{t-1}}{(1-e^{GRD})\hat G^{RD,SS}}",
    "S":        r"\hat S_t=\varsigma q_t\phi\,\mathbb{E}_t\!\big[\mathit{SDF}_{t+1}\tfrac{1}{1+\gamma^{a}}\tfrac{\hat A_{t-1}}{\hat A_t}(\hat{\mathcal V}_{t+1}-\hat{\mathcal J}_{t+1})\big]",
    "q":        r"q_t=(q_0+\varepsilon^{q}_t)(\hat S_t)^{\varsigma}",
    "V":        r"\hat{\mathcal V}_t=\tfrac{\mu^p-1}{\mu^p}mc_t\hat Y_t+\phi\,\mathbb{E}_t\big[\mathit{SDF}_{t+1}\hat{\mathcal V}_{t+1}\tfrac{\hat A_{t-1}}{\hat A_t}\tfrac{1}{1+\gamma^{a}}\big]",
    "J":        r"\hat{\mathcal J}_t=-\hat S_t+\phi\,\mathbb{E}_t\big[\mathit{SDF}_{t+1}\tfrac{\hat A_{t-1}}{\hat A_t}\tfrac{1}{1+\gamma^{a}}(q_t\hat{\mathcal V}_{t+1}+(1-q_t)\hat{\mathcal J}_{t+1})\big]",
    # Government
    "Gc":       r"\hat G^C_t=\hat G^{C,SS}+\hat Y^{d,SS}\varepsilon^{c}_t",
    "Igi":      r"\hat I^{GI}_t=\hat I^{GI,SS}+\hat Y^{d,SS}\varepsilon^{gi}_t",
    "Kg":       r"g\hat K^{GI}_t=(1-\delta)\hat K^{GI}_{t-1}+(1-e^{GI}_t)\hat I^{GI}_t",
    "Ige":      r"\hat I^{GE}_t=\hat I^{GE,SS}+\hat Y^{d,SS}\varepsilon^{ge}_t",
    "Kge":      r"g\hat K^{GE}_t=(1-\delta)\hat K^{GE}_{t-1}+(1-e^{GE}_t)\hat I^{GE}_t",
    "Grd":      r"\hat G^{RD}_t=\hat G^{RD,SS}+\hat Y^{d,SS}\varepsilon^{rd}_t",
    "G":        r"\hat G_t=\hat G^C_t+\hat I^{GI}_t+\hat I^{GE}_t+\hat G^{RD}_t",
    "b":        r"\hat B_t=\tfrac{R_{t-1}}{\Pi_t}\tfrac{\hat B_{t-1}}{g}+\hat G^C_t+\hat I^{GI}_t+\hat I^{GE}_t+\hat G^{RD}_t+\hat T_t-\tau^w_t\hat w_t N_t-\tau^c_t\hat C_t",
    "by":       r"b_t=\hat B_t/\hat Y^{d}_t",
    "T":        r"\hat T_t=\hat T^{SS}-\gamma^T_d e_{T,t}^{\mathrm{aux}}(b_{t-1}-b^*)\hat Y^{d,SS}",
    "tauc":     r"\tau^c_t=\tau^c+\varepsilon^{\tau c}_t",
    "tauw":     r"\tau^w_t=\tau^w+\varepsilon^{\tau w}_t",
    "R":        r"\tfrac{R_t}{R^{SS}}=\big(\tfrac{R_{t-1}}{R^{SS}}\big)^{\rho_R}\big[(\tfrac{\Pi_t}{\Pi})^{\gamma_\pi}(\tfrac{mc_t}{mc^{SS}})^{\gamma_y}\big]^{1-\rho_R}e^{\varepsilon^{mp}_t}",
    ...
    # Market clearing and equilibrium
    "yd":       r"\hat Y^d_t=\hat C_t+\hat I_t+\hat G^C_t+\hat I^{GI}_t+\hat I^{GE}_t+\hat G^{RD}_t+\big(\tfrac{\hat Z_{t-1}}{\hat A_{t-1}}-1\big)\hat S_t",
