...
def emit(groups, csv_out, tex_out, label, supplements=None, channel_effect=False):
    ...
    print(f"--- {label} ---")
    data = []   # (stype, region, model, mult-or-None, long-term channel effect)
    for stype, regions in groups:
        for region, model, inst in regions:
            ...
            for step, step_model, step_inst, _step_color in supplements.get(model, []):
                ...
                effect = mult[HORIZONS[-1]] - step_mult[HORIZONS[-1]] if channel_effect else None
    ...
    if channel_effect:
        csv_cols.append("longterm_channel_effect")
    ...
    ncol = len(COLS) + int(channel_effect)
    header_labels = [lbl for lbl, _ in COLS]
    if channel_effect:
        header_labels.append(r"\makecell{Long-term\\channel effect}")
    ...
    for gi, (stype, regions) in enumerate(groups):
        ...
        for region, model, inst in regions:
            ...
            for step, step_model, step_inst, step_color in supplements.get(model, []):
                ...
                if channel_effect:
                    effect = mult[HORIZONS[-1]] - step_mult[HORIZONS[-1]]
