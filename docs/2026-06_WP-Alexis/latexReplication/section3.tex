...
\begin{figure}[htbp]
  ...
  % --- Replication chart, stacked in the SAME float so it sits directly below
  %     Figure 3 on the same page. Unnumbered (\caption*) to avoid renumbering.
  \vspace{14pt}
  \caption*{\textbf{[Replication from source --- for comparison with Figure~3
  above]}}
  \subcap{SACU spending-efficiency gaps over time (eff\_gap, 0--1 scale), five
  countries vs.\ the global mean. Independent extract of the same efficiency-score
  data behind Figure~3.}
  \vspace{4pt}
  \includegraphics[width=\textwidth]{scoreEvolution_SACU_eff_gap.png}
  \figsource{Author calculations from the IMF Fiscal Monitor efficiency-score
  database (2025-04-14 estimates); see \texttt{chartReplication/}.}
\end{figure}
